...
class ExecutionInstance(InstanceResource):
    ...
    :ivar context: The current state of the Flow's Execution. As a flow executes, we save its state in this context. We save data that your widgets can access as variables.tf in configuration fields or in text areas as variable substitution.
    :ivar status: 
...
class ExecutionList(ListResource):
    ...
    def create(
        ...
        :param parameters: JSON data that will be added to the Flow's context and that can be accessed as variables.tf inside your Flow. For example, if you pass in `Parameters={\\\"name\\\":\\\"Zeke\\\"}`, a widget in your Flow can reference the variable `{{flow.data.name}}`, which returns \\\"Zeke\\\". Note: the JSON value must explicitly be passed as a string, not as a hash object. Depending on your particular HTTP library, you may need to add quotes or URL encode the JSON string.
